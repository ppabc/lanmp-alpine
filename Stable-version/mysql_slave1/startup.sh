...
if [ -d /data1/mysql ]; then
  ...
  mkdir -p /data1/mysql/ /data1/mysql/data/ /data1/mysql/logs/
  chown -R root:mysql /data1/mysql/
  mysql_install_db --user=root --datadir=/data1/mysql/data/ > /dev/null
...
EOF
  ...
  /usr/bin/mysqld --user=root --bootstrap --datadir=/data1/mysql/data/ --verbose=0 < $tfile
  rm -f $tfile
fi

exec /usr/bin/mysqld --user=root --datadir=/data1/mysql/data --console
